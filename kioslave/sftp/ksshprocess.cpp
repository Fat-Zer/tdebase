...
bool KSshProcess::connect() {
    ...
    while(--transitionLimit) {
        ...
        case STATE_WAIT_PROMPT:
            ...
            else if( line.tqfind(TQString::tqfromLatin1(passwordPrompt[mVersion]), 0, false) != -1 ) {
                mConnectState = STATE_TRY_PASSWD;
            }
            else if( line.tqfind(passphrasePrompt[mVersion]) != -1 ) {
                mConnectState = STATE_TRY_PASSPHRASE;
            }
            else if( line.tqfind(authSuccessMsg[mVersion]) != -1 ) {
                return true;
            }
            ...
            else if( line.tqfind(hostKeyMissingMsg[mVersion]) != -1 ) {
                mConnectState = STATE_NEW_KEY_WAIT_CONTINUE;
            }
            else if( line.tqfind(hostKeyChangedMsg[mVersion]) != -1 ) {
            ...
            else if( line.tqfind(changeHostKeyOnDiskPrompt[mVersion]) != -1 ) {
